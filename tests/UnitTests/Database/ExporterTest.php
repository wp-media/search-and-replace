...
class ExporterTest extends AbstractTestCase {
	...
	public function test_search_and_replace() {
		...
		Functions\when( 'get_temp_dir' )->justReturn( './' );
	...
	public function test_csv_data_string_replace() {
		...
		$dbm_mock->shouldReceive( 'get_table_content' )
		         ->andReturn(
			         [
				         [
					         "ID"                    => '',
					         "post_author"           => 'search_cv',
					         "post_date"             => '',
					         "post_date_gmt"         => '',
					         "post_content"          => 'search_cv',
					         "post_title"            => '',
					         "post_excerpt"          => '',
					         "post_status"           => 'search_cv',
					         "comment_status"        => '',
					         "ping_status"           => '',
					         "post_password"         => '',
					         "post_name"             => '',
		...
		$csv_data = 'search_cv,replace_cv';

		$instance = new Exporter( $replace_mock, $dbm_mock, $wp_error_mock );
		$response = $instance->backup_table( 'search', 'replace', 'wp_simple_table', 'new_table_prefix_', $csv_data );

		$this->assertSame( 3, $response['change'] );
	}

	public function test_csv_data_string_replace_without_searchandreplace_main_strings_should_works_also() {
		...
		$instance = new Exporter( $replace_mock, $dbm_mock, $wp_error_mock );
		$response = $instance->backup_table( '', '', 'wp_simple_table', 'new_table_prefix_', $csv_data );
